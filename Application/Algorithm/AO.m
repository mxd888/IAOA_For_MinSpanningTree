...
%_______________________________________________________________________________________%
function [AOsol,AObest,AOmeanbest]=AO(N,M_Iter,model)
...
T = M_Iter;
AObest = zeros(1,M_Iter);
AOmeanbest = zeros(1,M_Iter);
    ...
    for i=1:size(X,1)
        ...
        if t<=(2/3)*T
            if rand <0.5
                ...
                Xnew(i,:) = limitToPosition(Xnew(i,:), LB, UB);
                [Xnew(i,:),Ffun_new(1,i), FSol_new]=F_obj(Xnew(i,:));
                ...
                %-------------------------------------------------------------------------------------
                Xnew(i,:)=Best_P(1,:).*MyLevy(Dim)+X((floor(N*rand()+1)),:)+(y-x)*rand;       % Eq. (5)
                Xnew(i,:) = limitToPosition(Xnew(i,:), LB, UB);
                [Xnew(i,:),Ffun_new(1,i), FSol_new]=F_obj(Xnew(i,:));
                if Ffun_new(1,i)<Ffun(1,i)
                    X(i,:)=Xnew(i,:);
            ...
            if rand<0.5
                ...
                %-------------------------------------------------------------------------------------
                Xnew(i,:)=QF*Best_P(1,:)-(G2*X(i,:)*rand)-G1.*MyLevy(Dim)+rand*G2; % Eq. (14)
                Xnew(i,:) = limitToPosition(Xnew(i,:), LB, UB);
                [Xnew(i,:),Ffun_new(1,i), FSol_new]=F_obj(Xnew(i,:));
                if Ffun_new(1,i)<Ffun(1,i)
                    X(i,:)=Xnew(i,:);
